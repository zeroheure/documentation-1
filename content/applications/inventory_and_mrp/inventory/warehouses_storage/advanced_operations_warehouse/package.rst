...
Transfers* feature (:menuselection:`Inventory --> Configuration --> Settings`).

By default, *Package use* is set to *Disposable*. Change this field to *Reusable* **only** when
configuring packages for :ref:`cluster pickings <inventory/warehouses_storage/cluster-pack>`.
...
#. Clicking each *Detailed Operations* icon on the product line.
#. Using the *Put in Pack* button to place everything in the transfer into a package.

Detailed operations
-------------------

On any transfer, add a product to a package by clicking the :guilabel:`⦙≣ (bulleted list)` icon in
...
name of the package and select :guilabel:`Create...`.
...
Then, specify the quantity of items to go into the package in the :guilabel:`Done` field. Repeat the
above steps to place the :guilabel:`Product` in different packages. Once finished, click
...
Alternatively, click the :guilabel:`Put in Pack` button on any transfer to create a new package, and
place all the items in the transfer in that newly-created package.
...
custom dimensions and weight limits. This feature is mainly used to calculate package weights for
shipping costs.
...
.. note::
   ...
   the item(s) (from the :guilabel:`Weight` field in the :guilabel:`Inventory` tab of each product
   form).
...
.. seealso::
   :doc:`Using cluster packages <cluster_picking>`
