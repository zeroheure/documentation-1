...
.. tip::
   To hide the right sidebar, click the :guilabel:`(white and black box)` icon, located above the
   sidebar.
...
To the right of each rule, there are :guilabel:`➕ (Add node)`, :guilabel:`(Add branch)`, and
:guilabel:`🗑️ (Delete node)` icons.
...
.. note::
   The options available in the :guilabel:`Settings` tab vary depending on if the :guilabel:`Mailing
   Campaigns` feature is activated in the *Settings* page of the *Email Marketing* application
   (:menuselection:`Email Marketing --> Configuration --> Settings`).

Without the :guilabel:`Mailing Campaigns` feature activated, the :guilabel:`Settings` tab on the
email form only contains the :guilabel:`Preview Text`, :guilabel:`Send From`, :guilabel:`Reply To`,
...
.. warning::
   ...
   **all applications**. So, if there are remaining emails to be sent after a limit has been
   reached, those mailings are **not** sent automatically the next day. The sending needs to be
   forced, by opening the email and clicking :guilabel:`Retry`.
